 ...
 * @constructor
 */
const OnStudentExamResultPublished = async (result, context) => {
    const { app } = context;

    const { student, studentName, exam, status, _id: studentExamId, studentExamRoll } = result;
    ...
    if (status === 6) {
        ...
        if (studentEmail) {
            ...
            const message =
                `Hi ${studentName},` +
                '\n\n' +
                `Your ${title} Result at ${institute.name} has been published now.` +
                '\n\n' +
...
export default OnStudentExamResultPublished;
